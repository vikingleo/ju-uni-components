<!--
@FIXME 选择的数量等于{count}时，此时只要删除，造成可继续选择图片上传的状态，又可以选择{choose-count}，造成总数量超出{count}的情况
-->
<template>
    <view class="img-upload" :class="className" :style="{margin:count>1?gutter/2*-1+'px':0}">
        <view class="img-upload-content">
            <view class="img-upload-container img-preview-item" v-if="showImage" :key="item.id" v-for="(item,index) in instanceFiles" :style="{ flex:'0 0 '+instanceWidth, height:instanceHeight,padding:count>1?gutter/2+'px':0 }">
                <view class="img-upload-body" :style="{backgroundColor,}" :class="{square:rowCount>0}">
                    ...
                    <!--图片框-->
                    <view class="img-upload-body-content" :class="{'no-border':borderStyle==='none','selected':findInSelected(item.id)}" :style="{borderStyle,borderColor,borderWidth,borderRadius}">
                        <image :src="item.url" :mode="mode" class="img-upload-img" @click="onPreview(item,index)"></image>
                    </view>
                </view>
            </view>
            <view class="img-upload-container" v-if="(count===1&&!picUrls.length)||(picUrls.length<count)||!showImage" :style="{  flex:'0 0 '+instanceWidth, height:instanceHeight ,padding:count>1?(gutter/2+'px'):0 }">
...
https://cdn-static.yilumake.com/uploads/images/2021-08-03/d34a2b8bc3a0487753b8836f99accc25.png,
https://cdn-static.yilumake.com/uploads/images/2021-08-03/0bae386b2f7b3a50691ee6969e1ab655.png,
https://cdn-static.yilumake.com/uploads/images/2021-08-03/b9346e0f4d29aabbbc754c6cc2193dbe.png,
https://cdn-static.yilumake.com/uploads/images/2021-08-03/01bcaa00651882a72632a5d5a351fdb5.png
